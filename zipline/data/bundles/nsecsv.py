...
def nsecsvdir_equities(tframes=None, csvdir=None):
    """
    ...
    return NSECSVDIRBundle(tframes, csvdir).ingest


class NSECSVDIRBundle:
    """
...
@bundles.register("nsecsvdir")
def nsecsvdir_bundle(environ,
                  asset_db_writer,
                  minute_bar_writer,
                  daily_bar_writer,
                  adjustment_writer,
    ...
    """
    if not csvdir:
        csvdir = environ.get('NSECSVDIR')
        if not csvdir:
            raise ValueError("NSECSVDIR environment variable is not set")

    if not os.path.isdir(csvdir):
        raise ValueError("%s is not a directory" % csvdir)

    tmp_csv = "/tmp/csvdata"
    tranform_data(csvdir,tmp_csv)
    csvdir = tmp_csv
...
def tranform_data(csvdir,tmp_csv):
    months = os.listdir(csvdir)
...
def get_symbols(csvdir):
    parent= os.listdir(csvdir)[0]
    filename = os.listdir(csvdir+"/"+parent)[0]
    zipfile = ZipFile(open(csvdir+"/"+parent+'/'+filename))
    symbols = [a.filename.split("/")[1].split(".")[0] for a in zipfile.filelist]
    return symbols
